            ...
            if (k~=j || k~=0)  %The node itself is not its successor
                ...
                if( (s_x >0 && s_x <=MAX_X) && (s_y >0 && s_y <=MAX_Y))%node within array bound
                    ...
                    if (flag == 1)
                        ...
                        exp_array(exp_count,3) = hn+sqrt((node_x-s_x)^2+(node_y-s_y)^2);%cost of travelling to node
                        exp_array(exp_count,4) = sqrt((xTarget-s_x)^2+(yTarget-s_y)^2);%distance between node and goal
